...
plot5Data <- NEI_Baltimore[NEI_Baltimore$SCC %in% SCC_Index,]
plot5Data <- aggregate(Emissions ~ year, data=plot5Data, sum)

# Load ggplot2 library
library(ggplot2)

# Write to png file
png(file="plot5.png",width=600,height=600, bg="transparent")
ggplot(plot5Data,aes(y=Emissions,x=year)) + geom_line(size=1) + geom_point(size=3) +  xlab("Year") + ylab("PM2.5 emission") + ggtitle("PM2.5 emissions from motor vehicle sources in Baltimore City from 1999 to 2008") + theme(plot.title = element_text(lineheight=.8, face="bold"))
dev.off()